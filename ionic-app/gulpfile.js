...
gulp.task(
  "build",
  ["config", "sass", "html", "fonts", "scripts", "images", "ts"],
  function() {
    return buildBrowserify({
      src: [APP_MAIN, "./typings/index.d.ts"],
      outputPath: "www/build/js/"
...
gulp.task("e2e.run", function(done) {
  ...
  var endpoint =
    (process.env.E2E_CLIENT || "http://localhost:") +
    (process.env.E2E_PORT || 5555);
  return childProcess
    .spawn(
      "node",
      [
...
gulp.task("e2e.webserver", function(done) {
  e2eServerStream = gulp.src(DIST).pipe(
    webserver({
      host: "localhost",
      livereload: false,
...
gulp.task("webserver", function(done) {
  webserverStream = gulp.src(DIST).pipe(
    webserver({
