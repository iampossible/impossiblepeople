...
class PostModel extends Model {
  ...
  getComments(postID, userID) {
    ...
    postBelongsToUser(userID, postID) {
        return new Sequence((accept, reject) => {
            this.db.query(
                "MATCH(u:Person {userID: {userID}}) -[:OFFERS|:ASKS]->(p:Post {postID: {postID}}) RETURN p, count(*)", { userID, postID },
                (err, data) => {
                    if (err) return reject(err);
    ...
    reportComment(postID, commentID, userID) {
        return new Sequence((accept, reject) => {
         ...
         RETURN r`, { postID, userID, commentID },
                err => {
                    if (err) {
                        reject(err);
                    } else {
    ...
    deletePost(postID) {
        return new Sequence((accept, reject) => {
            this.db.query(
                "MATCH (p:Post {postID: {postID}}) DETACH DELETE p;", { postID },
                (err, data) => {
                    if (err) {
                        reject(err);
                    } else {
    ...
    resolvePost(postID) {
        return this.db.getOne(
            "MATCH (p:Post{postID: { postID }}) SET p.resolved = true RETURN p;", { postID }
        );
    }
