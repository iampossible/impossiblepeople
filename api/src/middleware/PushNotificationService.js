...
class PushNotificationService {
  pushNotification(notificationEndpoint, text, dataObj) {
    ...
    if (process.env.GNOME_ENV === "dev") {
      console.log(
        "[skiping SNS] PushNotificationService:",
        Object.assign({ notificationEndpoint, text }, dataObj)
      );
  ...
  unregister(user) {
    if (user.notificationEndpoint) {
      ...
      userModel
        .updateUser(user.userID, { notificationEndpoint: "" })
        .done(() => {
          // noop
        })
        .error(err => {
