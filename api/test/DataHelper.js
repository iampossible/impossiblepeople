...
class DataHelper {
  populateFrom(seedFile) {
    return new DataGenerator(require(seedFile)).parse().then(json =>
      this.wipe().then(next => {
        request(
          { method: "POST", uri: neo4jBatchEndpoint, body: json },
          (err, response) => {
            if (err) {
            ...
            if (response && response.attempts > 1)
              console.log(`populated after ${response.attempts} attempts`);
            next();
          }
        );
      })
  ...
  wipe() {
    return new Sequence(next => {
      ...
      request(
        { method: "POST", uri: neo4jQueryEndpoint, json },
        (err, response) => {
          if (err) {
          ...
          if (response && response.attempts > 1)
            console.log(`wiped after ${response.attempts} attempts`);
          next();
        }
      );
    });
